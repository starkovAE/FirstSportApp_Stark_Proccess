...
class ProfileViewController: UIViewController {
    ...
    private let editingButton: UIButton = {
        ...
        button.setImage(UIImage(named: "space"), for: .normal)
    ...
    private var resultWorkout = [ResultWorkout]() //массив из структур
    //MARK: -  viewWillAppear
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        profileArray = localRealm.objects(ProfileModel.self)
        setupViews()
        setConstraints()
        setDelegates()
        getWorkoutResults()
    ...
    @objc private func editingButtonTap() {
        ...
        present(editingProfileVC, animated: true, completion: nil)
    }
    private func setupUserParameters() { //устанавливали значения для профиля
        if profileArray.count != 0 {
            userNameLabel.text = "\(profileArray[0].userFirstName)  \(profileArray[0].userSecondName)" //[0] чтобы всегда было новое значение (самое свежее)а не последнее
            userHeightLabel.text = "Height: \(profileArray[0].userHeight)"
            userWeightLabel.text = "Weight: \(profileArray[0].userWeight)"
            targetLabel.text = "TARGET: \(profileArray[0].userTarget) workouts"
            workoutsTargetLabel.text = "\(profileArray[0].userTarget)"
            guard let data  = profileArray[0].userImage else { return } // из  массива свойства userImage записываем в  data - данные
            guard let image = UIImage(data: data) else { return } // делаем из нее UIImage
            userPhotoImageView.image = image //установили наше имедж в свойство этого класса
        }
    }
    private func getWorkoutName() -> [String] { //метод для получения имени тренировки из модели, но чтобы они не повторялись
    ...
    private func getWorkoutResults() {
        ...
        for name in nameArray {
            let predicateName = NSPredicate(format: "workoutName = '\(name)'") //создаем предикат, по формату предката мы ниже будет фильтровать
            workoutArray = localRealm.objects(WorkoutModel.self).filter(predicateName).sorted(byKeyPath: "workoutName")
            var result = 0
            var image: Data?
            workoutArray.forEach { model in //forEach - для каждого элемента из workoutArray
                result += model.workoutReps //мы будем прибавлять количество повторений
                image = model.workoutImage // и добавим изображение
            }
            let resultModel = ResultWorkout(name: name, result: result, imageData: image) // создадим экземпляр структры
            resultWorkout.append(resultModel) // заполним наш массив этими экземплярами
        }
    }
}
...
extension ProfileViewController: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let model = resultWorkout[indexPath.row] //модель представляет собой массив структур
        cell.cellConfigure(model: model) //передаем модель 
        cell.backgroundColor = (indexPath.row % 4 == 0 || indexPath.row % 4 == 3 ? .specialGreen : .specialDarkYellow) //конфигурирование цвета фона
        return cell
    }
}
